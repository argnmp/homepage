    ...
    let createCategory = (list, isUpper, upperPage) => {
        ...
        for(let i in list){
            if(typeof(list[i])=='object'){        
                ...
                returnElement.push(<li key={i} onClick={(e)=>{e.stopPropagation(); document.location.href=targetUrl;}}><span>{i}</span>{createCategory(list[i], false, i)}</li>)        
                //stopPropagation이 없다면 onclick 이벤트가 버블링 되어서 상위의 카테고리가 마지막으로 눌리게 됨. 이것때문에 많이 고생함..
            }else{

                const targetUrl = getUrl(i,list[i],upperPage);
                returnElement.push(<li key={i} onClick={(e)=>{e.stopPropagation(); document.location.href=targetUrl;}}><span>{i}</span></li>);
            }
        }
        if(isUpper){
